...
import (
	...
	feedbackv1 "peoplesuite/platform-contracts/gen/go/feedback/v1"

	sdkgrpc "github.com/peoplesuite/platform-sdk-go/pkg/grpc"
)

// Config configures a Feedback client.
type Config struct {
	Address string
	Logger  *zap.Logger
...
// Client is a typed wrapper around feedbackv1.FeedbackServiceClient.
type Client struct {
	grpc   feedbackv1.FeedbackServiceClient
	conn   *grpc.ClientConn
	config Config
}

// New creates a new Feedback client using the provided Config.
func New(ctx context.Context, cfg Config) (*Client, error) {
	...
	return &Client{
		grpc:   feedbackv1.NewFeedbackServiceClient(conn),
		conn:   conn,
		config: cfg,
	}, nil
}
...
func (c *Client) GRPC() feedbackv1.FeedbackServiceClient {
	if c == nil {
		return nil
	}
	return c.grpc
